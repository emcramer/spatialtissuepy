...
class TestPhysiCellHelpers:
    ...
    def test_is_alive_function(self):
        ...
        assert is_alive(14) is True  # 'live' phase
        assert is_alive(5) is True   # G0 phase

        # Dead phases (>= 100)
        assert is_alive(100) is False  # apoptotic
        assert is_alive(103) is False  # necrotic

    def test_is_dead_function(self):
        """Test is_dead helper function."""
        # Live phases
        assert is_dead(0) is False
        assert is_dead(14) is False

        # Dead phases
        assert is_dead(100) is True
        assert is_dead(101) is True
        assert is_dead(104) is True  # debris
